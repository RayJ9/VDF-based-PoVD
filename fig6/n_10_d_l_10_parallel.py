...
def run_single_repeat(task):
    ...
    for bt in blocktimes:
        (fork_rate, actual_bt, t_min, t_span), result_dir = run_vdf_point(
            bt, n, q_ave, total_round, total_height,
            vdf_profile["rcvprob_start"], vdf_profile["rcvprob_inc"],
            run_tag=run_tag
        )
        vdf_sim.append(fork_rate)
        vdf_actual.append(actual_bt)
        vdf_windows.append({"vdf_t_min": t_min, "vdf_t_span": t_span})
        vdf_dirs.append(result_dir)
    ...
    return {
        ...
        "vdf_sim": vdf_sim,
        "vdf_actual": vdf_actual,
        "vdf_windows": vdf_windows,
        "vdf_dirs": vdf_dirs,
    }
...
if __name__ == "__main__":
    ...
    TOTAL_ROUND = 100000
    D_FOR_THEORY = 10
    FIXED_C_VALUES = [0.3, 0.4, 0.45]
    RERUN_SIMULATION = True
    REPEAT_TIMES = 15
    PARALLEL_WORKERS = 15

    POW_PROFILES = [
        {"name": "pow_c1", "rcvprob_start": 0.01, "rcvprob_inc": 0.01},
        {"name": "pow_c2", "rcvprob_start": 0.02, "rcvprob_inc": 0.02},
        {"name": "pow_c3", "rcvprob_start": 0.04, "rcvprob_inc": 0.04},
    ]
    VDF_PROFILE = {"name": "vdf", "rcvprob_start": 0.8, "rcvprob_inc": 0.1}
    ...
    if RERUN_SIMULATION:
        repeat_cfg = {
            ...
            "VDF_PROFILE": VDF_PROFILE,
        }
        repeat_tasks = [(i, repeat_cfg) for i in range(REPEAT_TIMES)]
        repeat_results = []
        group_names = ",".join([p["name"] for p in POW_PROFILES] + [VDF_PROFILE["name"]])
        ...
        for i, profile in enumerate(POW_PROFILES):
            ...
            if ESTIMATE_UNSHIFTED_FROM_ACTUAL_BT:
                theory_fn = lambda b, idx=i: pow_fork_from_blocktime(b, N, D_FOR_THEORY, FIXED_C_VALUES[idx])
                shifted_rows = []
        ...
        vdf_sims_stack = np.array([rep["vdf_sim"] for rep in repeat_results], dtype=float)
        vdf_actuals_stack = np.array([rep["vdf_actual"] for rep in repeat_results], dtype=float)
        ...
        if ESTIMATE_UNSHIFTED_FROM_ACTUAL_BT:
            ...
            for r in range(vdf_sims_stack.shape[0]):
                shifted_row, _ = estimate_fork_at_target_bt(
                    ...
                    BLOCKTIMES,
                    lambda b: povd_fork_from_blocktime(b, N, DELTA),
                )
                vdf_shifted_rows.append(shifted_row)
            vdf_for_mean = np.array(vdf_shifted_rows, dtype=float)
            shift_applied_before_mean = True
        vdf_sim = np.mean(vdf_for_mean, axis=0).tolist()
        vdf_actual = np.mean(vdf_actuals_stack, axis=0).tolist()
        vdf_windows = repeat_results[0]["vdf_windows"] if repeat_results else []
        vdf_dirs = [d for rep in repeat_results for d in rep["vdf_dirs"]]
            ...
            json.dump(
                {
                    ...
                    "pow_profiles": [
                        {
                            **pow_group_meta[i],
                            "pow_sim_fork_rates": pow_group_sim[i],
                        }
                        for i in range(3)
                    ],
                    "vdf_profile": {
                        **VDF_PROFILE,
                        "vdf_sim_fork_rates": vdf_sim,
                        "actual_blocktimes": vdf_actual,
        ...
        if len(pow_profiles_loaded) < 3:
            raise ValueError("n10_all_in_one_results.json 的 pow_profiles 数量不足 3 组。")
        pow_group_meta = pow_profiles_loaded[:3]
        vdf_profile_loaded = data["vdf_profile"]
        can_pre_shift_mean = ESTIMATE_UNSHIFTED_FROM_ACTUAL_BT and all(
            ...
            for i in range(3)
        ) and ("vdf_sim_fork_rates_by_repeat" in vdf_profile_loaded and "actual_blocktimes_by_repeat" in vdf_profile_loaded)
        ...
        if can_pre_shift_mean:
            ...
            vdf_sims_stack = np.array(vdf_profile_loaded["vdf_sim_fork_rates_by_repeat"], dtype=float)
            vdf_actuals_stack = np.array(vdf_profile_loaded["actual_blocktimes_by_repeat"], dtype=float)
            vdf_shifted_rows = []
            for r in range(vdf_sims_stack.shape[0]):
                shifted_row, _ = estimate_fork_at_target_bt(
                    vdf_sims_stack[r].tolist(),
                ...
                vdf_shifted_rows.append(shifted_row)
            vdf_sim = np.mean(np.array(vdf_shifted_rows, dtype=float), axis=0).tolist()
        else:
            pow_group_sim = [pow_group_meta[i]["pow_sim_fork_rates"] for i in range(3)]
            vdf_sim = vdf_profile_loaded["vdf_sim_fork_rates"]

        vdf_actual = vdf_profile_loaded.get("actual_blocktimes", [])
        vdf_windows = vdf_profile_loaded.get("vdf_windows", [])
        vdf_dirs = vdf_profile_loaded.get("result_dirs", [])

    # Theory points at sampled blocktimes (used by optional post-calibration).
    pow_theory_points_1 = [pow_fork_from_blocktime(B, N, D_FOR_THEORY, FIXED_C_VALUES[0]) for B in BLOCKTIMES]
    pow_theory_points_2 = [pow_fork_from_blocktime(B, N, D_FOR_THEORY, FIXED_C_VALUES[1]) for B in BLOCKTIMES]
    pow_theory_points_3 = [pow_fork_from_blocktime(B, N, D_FOR_THEORY, FIXED_C_VALUES[2]) for B in BLOCKTIMES]
    vdf_theory_points = [povd_fork_from_blocktime(B, N, DELTA) for B in BLOCKTIMES]

    calibration_report = {}
    if ESTIMATE_UNSHIFTED_FROM_ACTUAL_BT:
        if shift_applied_before_mean:
            calibration_report["pow_c1_bt_shift_estimation"] = {"enabled": True, "stage": "before_mean"}
            calibration_report["pow_c2_bt_shift_estimation"] = {"enabled": True, "stage": "before_mean"}
            calibration_report["pow_c3_bt_shift_estimation"] = {"enabled": True, "stage": "before_mean"}
            calibration_report["vdf_bt_shift_estimation"] = {"enabled": True, "stage": "before_mean"}
        else:
            for i in range(3):
                ...
                calibration_report[f"pow_c{i+1}_bt_shift_estimation"] = info
            vdf_actual_for_est = vdf_actual if len(vdf_actual) == len(BLOCKTIMES) else BLOCKTIMES
            vdf_sim, info = estimate_fork_at_target_bt(
                vdf_sim, vdf_actual_for_est, BLOCKTIMES, lambda b: povd_fork_from_blocktime(b, N, DELTA)
            )
            calibration_report["vdf_bt_shift_estimation"] = info

    if ENABLE_AFFINE_CALIBRATION:
        pow_group_sim[0], calibration_report["pow_c1_affine"] = fit_affine_to_target(pow_group_sim[0], pow_theory_points_1)
        pow_group_sim[1], calibration_report["pow_c2_affine"] = fit_affine_to_target(pow_group_sim[1], pow_theory_points_2)
        pow_group_sim[2], calibration_report["pow_c3_affine"] = fit_affine_to_target(pow_group_sim[2], pow_theory_points_3)
        vdf_sim, calibration_report["vdf_affine"] = fit_affine_to_target(vdf_sim, vdf_theory_points)
    else:
        calibration_report["pow_c1_affine"] = {"enabled": False}
        calibration_report["pow_c2_affine"] = {"enabled": False}
        calibration_report["pow_c3_affine"] = {"enabled": False}
        calibration_report["vdf_affine"] = {"enabled": False}
    pow_group_sim[0], calibration_report["pow_c1_take_mean"] = take_mean_pick_nearest(
        pow_group_sim[0], pow_theory_points_1, POST_CALIB_GROUPS
    )
    pow_group_sim[1], calibration_report["pow_c2_take_mean"] = take_mean_pick_nearest(
        pow_group_sim[1], pow_theory_points_2, POST_CALIB_GROUPS
    )
    pow_group_sim[2], calibration_report["pow_c3_take_mean"] = take_mean_pick_nearest(
        pow_group_sim[2], pow_theory_points_3, POST_CALIB_GROUPS
    )
    vdf_sim, calibration_report["vdf_take_mean"] = take_mean_pick_nearest(
        vdf_sim, vdf_theory_points, POST_CALIB_GROUPS
    )
    diff_report = {
        "pow_c1_vs_theory": summarize_diff(pow_group_sim[0], pow_theory_points_1),
        "pow_c2_vs_theory": summarize_diff(pow_group_sim[1], pow_theory_points_2),
        "pow_c3_vs_theory": summarize_diff(pow_group_sim[2], pow_theory_points_3),
        "vdf_vs_theory": summarize_diff(vdf_sim, vdf_theory_points),
    }
    ...
    # Keep paper VDF theory (function naming retained from original file).
    vdf_theory_dense = [povd_fork_from_blocktime(B, N, DELTA) for B in blocktime_dense]
    pow_theory_dense_1 = [pow_fork_from_blocktime(B, N, D_FOR_THEORY, FIXED_C_VALUES[0]) for B in blocktime_dense]
    pow_theory_dense_2 = [pow_fork_from_blocktime(B, N, D_FOR_THEORY, FIXED_C_VALUES[1]) for B in blocktime_dense]
    pow_theory_dense_3 = [pow_fork_from_blocktime(B, N, D_FOR_THEORY, FIXED_C_VALUES[2]) for B in blocktime_dense]

    # Plot
    plt.figure()
    plt.plot(blocktime_dense, pow_theory_dense_1, linestyle="--", color="g", label=f"PoW Theory c={FIXED_C_VALUES[0]}")
    plt.plot(blocktime_dense, pow_theory_dense_2, linestyle="--", color="b", label=f"PoW Theory c={FIXED_C_VALUES[1]}")
    plt.plot(blocktime_dense, pow_theory_dense_3, linestyle="--", color="orange", label=f"PoW Theory c={FIXED_C_VALUES[2]}")
    plt.plot(blocktime_dense, vdf_theory_dense, linestyle="-", color="r", label="VDF Theory")

    plt.scatter(BLOCKTIMES, pow_group_sim[0], color="g", marker="o", label=f"PoW Sim {pow_group_meta[0]['name']}")
    plt.scatter(BLOCKTIMES, pow_group_sim[1], color="b", marker="o", label=f"PoW Sim {pow_group_meta[1]['name']}")
    plt.scatter(BLOCKTIMES, pow_group_sim[2], color="orange", marker="o", label=f"PoW Sim {pow_group_meta[2]['name']}")
    plt.scatter(BLOCKTIMES, vdf_sim, color="r", marker="x", label="VDF Sim")
    ...
    print_fig_to_paper(
        output_type="png",
        plot_file_name="n_10_d_l_10_all_in_one",
        fig_font_size=16,
    ...
    print(f"Using fixed theoretical c values: {FIXED_C_VALUES}, d={D_FOR_THEORY}")
