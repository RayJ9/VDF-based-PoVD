...
def plot_mining_power_timeline(all_results, duration, loop_index):
    ...
    for slice_label, (start_fraction, end_fraction) in slices.items():
        ...
        plot_file_name = f"{output_folder}/power_timeline_{duration}s_loop{loop_index}_{timestamp}_{slice_label}"
...
if __name__ == "__main__":
    ...
    core_counts = [16]
    modes = ["PoW", "VDF_Baseline", "PoVD"]
    duration_per_run = 150
    record_duration = 120
    total_loops = 1
    if IDLE_POWER_WATTS is None:
        IDLE_POWER_WATTS = 30.0 if CPU_TDP_WATTS is None else max(20.0, float(CPU_TDP_WATTS) * 0.2)
